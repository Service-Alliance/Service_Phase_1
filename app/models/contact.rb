class Contact < ActiveRecord::Base
 include Trackable
  ...
  enum category: {contact: 0, agent: 1, adjuster: 2, caller: 3, occupant: 4, property_manager: 5}
  ...
  def self.same_as_caller(job)
    ...
    if @job.contact_id
      ...
      contact.find_by(id: @job.contact_id).destroy
    end
    ...
    @caller_phones.each do |phone|
      @contact.phones.create(number: phone.number, extension: phone.extension, type_id: phone.type_id)
    end
